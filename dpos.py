...
def translate_seq_no_g_hpos(sequence: str, seq_index: int) -> list[str]:
    res: list[str] = []
    s_count: int = 1
    for c in sequence:
        if c != '-':
...
def translate_profile_no_g_naming(profile: list[str]) -> list[list[str]]:
    res: list[list[str]] = []
    for index, seq in enumerate(profile):
        res.append(translate_seq_no_g_hpos(seq, index + 1))
    return res
...
def get_place_dpos_no_gap(set_a: set, set_b: set) -> float:
    symmetric_diff_size = len(set_a.difference(set_b)) + len(set_b.difference(set_a))
...
def create_hpos_no_g_table(profile_naming: list[list[str]]) -> list[list[set[str]]]:
    seq_len: int = len(profile_naming[0])
    seq_count: int = len(profile_naming)
    hpos_table: list[list[set[str]]] = [[] for i in range(seq_count)]
    for col_index in range(seq_len):
...
def compute_dpos_no_g_distance(profile_a: list[str], profile_b: list[str]) -> float:
    dpos_list: list[float] = []
    profile_a_naming: list[list[str]] = translate_profile_no_g_naming(profile_a)
    profile_b_naming: list[list[str]] = translate_profile_no_g_naming(profile_b)
    seq_count: int = len(profile_a)
    profile_a_hpos: list[list[set[str]]] = create_hpos_no_g_table(profile_a_naming)
    profile_b_hpos: list[list[set[str]]] = create_hpos_no_g_table(profile_b_naming)
    for i in range(seq_count):
        for j in range(len(profile_a_hpos[i])):
            hpos_a_i: set[str] = profile_a_hpos[i][j]
            hpos_b_i: set[str] = profile_b_hpos[i][j]
            if len(hpos_a_i) > 0 or len(hpos_b_i) > 0:
                dpos_i_j = get_place_dpos_no_gap(hpos_a_i, hpos_b_i)
                dpos_list.append(dpos_i_j)
            else:
                dpos_list.append(0)
    if len(dpos_list) > 0:
