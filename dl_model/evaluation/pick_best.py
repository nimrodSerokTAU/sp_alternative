...
from dl_model.data_processing.io import read_features as _read_features_into_df
from dl_model.config.config import TrueScoreName
from dl_model.config.constants import SOP_SCORE_NAME, PREDICTED_SCORE_NAME, PREDICTED_CLASS_PROB
...
class PickBest:
    def __init__(self,
                 ...
                 sum_of_pairs_score: str = SOP_SCORE_NAME,
                 error: float = 0.0,
                 subset: float = None,
                 output_dir: str = '../out'
                 ) -> None:
    ...
    def save_to_csv(self, i: int):
        filename = f'{self.output_dir}/pick_me_trio_v{i}.csv'
        self.winners_df_w_code.to_csv(filename, index=False)
        filename = f'{self.output_dir}/pick_me_trio_overall_v{i}.csv'
        self.overall_winners_df_w_code.to_csv(filename, index=False)

    def plot_results(self, i: int):
        plotname = f'{self.output_dir}/pick_me_trio_plot_v{i}.png'
        df = self.winners_df
...
class PickMeAggregator:
    ...
    def add(self, pick_me_run: PickBest) -> None:
    ...
    def save_to_csv(self):
        ...
        filename = f'{self.output_dir}/pick_me_Results_ALL_{leng}.csv'
        self.pickme_df.to_csv(filename, index=False)
        leng = len(self.winners_df)
        filename = f'{self.output_dir}/pick_me_Winners_ALL_{leng}.csv'
        self.winners_df.to_csv(filename, index=False)
        leng = len(self.overall_winners_df)
        filename = f'{self.output_dir}/pick_me_overall_Winners_ALL_{leng}.csv'
    ...
    def plot_results(self):
        plotname = f'{self.output_dir}/pick_me_trio_plot_ALL.png'
        df = self.winners_df
