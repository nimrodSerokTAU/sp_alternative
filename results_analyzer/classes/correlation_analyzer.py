...
class CorrelationAnalyzer:
    ...
    def __init__(self, working_dir: str, relative_file_paths: list[str], label_name: str, measures: list[Measure],
                 example_dataset: str, print_zoom: bool, multi_dataset_title: str, points_count: int,
        ...
        for measure in self.measures:
            print(
                f'top_v of {measure.key} avg r is {sum(measure.r_values) / max(len(measure.r_values), 1)} for {len(measure.r_values)} samples')
    ...
    def arrange_data(self, datasets_list: list[DataSet], continue_missing_code: bool):
        ...
        for measure_i in range(len(self.measures)):
            ...
            self.data_group_mgr.set_measure_direction(self.measures[measure_i].key,
                                                      self.measures[measure_i].correlation_direction == 1)
            for rd in self.datasets_list:
