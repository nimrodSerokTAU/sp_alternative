...
class PickMeGameProgramMax:
    def __init__(self, features_file: str, prediction_file: str, predicted_measure: Literal['msa_distance', 'tree_distance'] = 'msa_distance', error: float = 0.0) -> None:
        ...
        if predicted_measure == "msa_distance":
            self.true_score = 'MEAN_COL_SCORE'
            # self.true_score = 'MEAN_RES_PAIR_SCORE'
        ...
        for code in df['code1'].unique():
            ...
            if not mafft_df.empty:
                min_mafft_row = mafft_df.loc[mafft_df[self.true_score].idxmax()]
                min_mafft_true_score = min_mafft_row[self.true_score]
                min_mafft_code_filename = min_mafft_row['code']

                min_mafft_predicted_score_row = mafft_df.loc[mafft_df[self.predicted_score].idxmax()]
                ...
                sorted_mafft_temp_df = mafft_df.sort_values(by=self.predicted_score, ascending=False)
            ...
            if not prank_df.empty:
                min_prank_row = prank_df.loc[prank_df[self.true_score].idxmax()]
                min_prank_true_score = min_prank_row[self.true_score]
                min_prank_code_filename = min_prank_row['code']

                min_prank_predicted_score_row = prank_df.loc[prank_df[self.predicted_score].idxmax()]
                ...
                sorted_prank_temp_df = prank_df.sort_values(by=self.predicted_score, ascending=False)
            ...
            if not muscle_df.empty:
                min_muscle_row = muscle_df.loc[muscle_df[self.true_score].idxmax()]
                min_muscle_true_score = min_muscle_row[self.true_score]
                min_muscle_code_filename = min_muscle_row['code']

                min_muscle_predicted_score_row = muscle_df.loc[muscle_df[self.predicted_score].idxmax()]
                ...
                sorted_muscle_temp_df = muscle_df.sort_values(by=self.predicted_score, ascending=False)
            ...
            if not baliphy_df.empty:
                min_baliphy_row = baliphy_df.loc[baliphy_df[self.true_score].idxmax()]
                min_baliphy_true_score = min_baliphy_row[self.true_score]
                min_baliphy_code_filename = min_baliphy_row['code']

                min_baliphy_predicted_score_row = baliphy_df.loc[baliphy_df[self.predicted_score].idxmax()]
                ...
                sorted_baliphy_temp_df = baliphy_df.sort_values(by=self.predicted_score, ascending=False)
            ...
            min_true_score_row = code_df.loc[code_df[self.true_score].idxmax()]
            ...
            min_predicted_score_row = code_df.loc[code_df[self.predicted_score].idxmax()]
            ...
            sorted_temp_df = code_df.sort_values(by=self.predicted_score, ascending=False)
    ...
    def plot_results(self, i: int):
        ...
        #     'Out of MAFFT alternatives could we choose better than default?': (df['min_predicted_le_default_mafft'].mean(skipna=True) * 100),
        #     'Out of PRANK alternatives could we choose better than default?': (df['min_predicted_le_default_prank'].mean(skipna=True) * 100),
        #     'Out of MUSCLE alternatives could we choose better than default?': (df['min_predicted_le_default_muscle'].mean(skipna=True) * 100),
        #     'Out of Bali-Phy alternatives could we choose better than default?': (df['min_predicted_le_default_baliphy'].mean(skipna=True) * 100),
        #     'Is "best predicted" better than the best out of MAFFT Alternatives?': (df['min_predicted_le_min_mafft'].mean(skipna=True) * 100),
        #     'Is "best predicted" better than the best out of PRANK Alternatives?': (df['min_predicted_le_min_prank'].mean(skipna=True) * 100),
        #     'Is "best predicted" better than the best out of MUSCLE Alternatives?': (df['min_predicted_le_min_muscle'].mean(skipna=True) * 100),
        ...
        percentages = {
            'Did we find the minimum !score?': (df['min_true_equals_min_predicted'].apply(lambda x: 1 if x == True else 0).sum() / df['min_true_equals_min_predicted'].notna().sum() * 100),
            'Out of MAFFT alternatives could we choose better than default?': (df['min_predicted_le_default_mafft'].apply(lambda x: 1 if x == True else 0).sum() / df['min_predicted_le_default_mafft'].notna().sum() * 100),
            'Out of PRANK alternatives could we choose better than default?': (df['min_predicted_le_default_prank'].apply(lambda x: 1 if x == True else 0).sum() / df['min_predicted_le_default_prank'].notna().sum() * 100),
            'Out of MUSCLE alternatives could we choose better than default?': (df['min_predicted_le_default_muscle'].apply(lambda x: 1 if x == True else 0).sum() / df['min_predicted_le_default_muscle'].notna().sum() * 100),
            'Out of Bali-Phy alternatives could we choose better than default?': (df['min_predicted_le_default_baliphy'].apply(lambda x: 1 if x == True else 0).sum() / df['min_predicted_le_default_baliphy'].notna().sum() * 100),
            'Is "best predicted" better than the best out of MAFFT Alternatives?': (df['min_predicted_le_min_mafft'].apply(lambda x: 1 if x == True else 0).sum() / df['min_predicted_le_min_mafft'].notna().sum() * 100),
            'Is "best predicted" better than the best out of PRANK Alternatives?': (df['min_predicted_le_min_prank'].apply(lambda x: 1 if x == True else 0).sum() / df['min_predicted_le_min_prank'].notna().sum() * 100),
            'Is "best predicted" better than the best out of MUSCLE Alternatives?': (df['min_predicted_le_min_muscle'].apply(lambda x: 1 if x == True else 0).sum() / df['min_predicted_le_min_muscle'].notna().sum() * 100),
            'Is "best predicted" better than the best out of Bali-Phy Alternatives?': (df['min_predicted_le_min_baliphy'].apply(lambda x: 1 if x == True else 0).sum() / df['min_predicted_le_min_baliphy'].notna().sum() * 100),
            'Was the real minimum !score among our (predicted) top 20 true scores?': (df['min_true_in_top20_min_predicted'].apply(lambda x: 1 if x == True else 0).sum() / df['min_true_in_top20_min_predicted'].notna().sum() * 100),
        }
    ...
    def plot_SoP_results(self, i: int):
        ...
        percentages = {
            'Did SoP find the minimum !score?': (df['min_true_equals_max_SoP'].apply(lambda x: 1 if x == True else 0).sum() / df['min_true_equals_max_SoP'].notna().sum() * 100),
            'Out of MAFFT alternatives did SoP helped to choose better than default?': (df['max_SoP_le_default_mafft'].apply(lambda x: 1 if x == True else 0).sum() / df['max_SoP_le_default_mafft'].notna().sum() * 100),
            'Out of PRANK alternatives did SoP helped to choose better than default?': (df['max_SoP_le_default_prank'].apply(lambda x: 1 if x == True else 0).sum() / df['max_SoP_le_default_prank'].notna().sum() * 100),
            'Out of MUSCLE alternatives did SoP helped to choose better than default?': (df['max_SoP_le_default_muscle'].apply(lambda x: 1 if x == True else 0).sum() / df['max_SoP_le_default_muscle'].notna().sum() * 100),
            'Out of Bali-Phy alternatives did SoP helped to choose better than default?': (df['max_SoP_le_default_baliphy'].apply(lambda x: 1 if x == True else 0).sum() / df['max_SoP_le_default_baliphy'].notna().sum() * 100),
            'Is "best SoP" choice better than the best out of all MAFFT Alternatives?': (df['max_SoP_le_min_mafft'].apply(lambda x: 1 if x == True else 0).sum() / df['max_SoP_le_min_mafft'].notna().sum() * 100),
            'Is "best SoP" choice better than the best out of all PRANK Alternatives?': (df['max_SoP_le_min_prank'].apply(lambda x: 1 if x == True else 0).sum() / df['max_SoP_le_min_prank'].notna().sum() * 100),
            'Is "best SoP" choice better than the best out of all MUSCLE Alternatives?': (df['max_SoP_le_min_muscle'].apply(lambda x: 1 if x == True else 0).sum() / df['max_SoP_le_min_muscle'].notna().sum() * 100),
            'Is "best SoP" choice better than the best out of all Bali-Phy Alternatives?': (df['max_SoP_le_min_baliphy'].apply(lambda x: 1 if x == True else 0).sum() / df['max_SoP_le_min_baliphy'].notna().sum() * 100),
            'Was the real minimum !score among SoP top 20 true scores?': (df['min_true_in_top20_max_SoP'].apply(lambda x: 1 if x == True else 0).sum() / df['min_true_in_top20_max_SoP'].notna().sum() * 100),
        }
