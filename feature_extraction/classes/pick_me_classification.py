...
class PickMeGameClassification:
    ...
    #         else:
    #             min_predicted_score_row = df.loc[df['sop_score'].idxmax()]
    #         min_predicted_true_score = min_predicted_score_row[self.true_score]
    #         scores.append(min_predicted_true_score)
    #         scores.append(max_SoP_true_score)
    #     else:
    ...
    def run(self, i):
        ...
        for code in df['code1'].unique():
            ...
            # remove all TRUE and extra alternatives
            ...
            default_mafft_true_score = default_mafft_true_scores[0] if len(default_mafft_true_scores) > 0 else np.nan #TODO check that nan's are not counted as lowest scores
            ...
            substrings = ['muscle', 'prank', '_TRUE.fas', 'true_tree.txt', 'bali_phy', 'BALIPHY', 'original', 'concat', "_alt"]
            mask = code_df['code'].str.contains('|'.join(substrings), case=False, na=False)
    ...
    def save_to_csv(self, i: int):
        filename = f'./out/pick_me_classification_v{i}.csv'
        self.winners_df_w_code.to_csv(filename, index=False)
        filename = f'./out/pick_me_classification_overall_v{i}.csv'
        self.overall_winners_df_w_code.to_csv(filename, index=False)

    def plot_results(self, i: int):
        plotname = f'./out/pick_me_classification_plot_v{i}.png'
        df = self.winners_df
    ...
    def plot_overall_results(self, i: int):
        plotname = f'./out/pick_me_classification_overall_plot_v{i}.png'
        df = self.overall_winners_df
        legend = ["predicted", "sop", "default", 'tie(predicted and sop)']  # Add 'D' to ensure it gets counted as 0 if missing
        counts = df.apply(lambda x: x.value_counts()).fillna(0)
...
#         self.overall_win = []
#
#     def add(self, pick_me_run: PickMeGameClassification):
...
#         length = len(self.winners_df)
#         filename = f'./out/pick_me_Winners_ALL_{length}.csv'
#         self.winners_df.to_csv(filename, index=False)
#         length = len(self.overall_winners_df)
#         filename = f'./out/pick_me_overall_Winners_ALL_{length}.csv'
...
#         plotname = f'./out/pick_me_classification_plot_ALL.png'
#         df = self.winners_df
